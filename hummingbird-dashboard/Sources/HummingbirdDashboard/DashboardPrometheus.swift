...
public struct PrometheusExporter: Sendable {
    ...
    public func render(_ snapshot: DashboardSnapshot) -> String {
        ...
        metric(
            "http_request_errors_total", "Total number of HTTP responses with status >= 400.", "counter",
            [("", String(snapshot.totalErrors))]
        )
        metric(
            "http_requests_in_flight", "Number of requests currently being processed.", "gauge",
            [("", String(snapshot.inFlight))]
        )
        metric(
            "http_requests_per_second", "Requests per second averaged over the last 60 seconds.", "gauge",
            [("", String(format: "%.3f", snapshot.requestsPerSecond))]
        )
        metric(
            "http_request_duration_seconds", "Request duration percentiles.", "summary",
            [
        ...
        metric(
            "http_response_size_bytes_total", "Total bytes sent in response bodies.", "counter",
            [("", String(snapshot.dataOutBytes))]
        )
        metric(
            "http_request_size_bytes_total", "Total bytes received in request bodies.", "counter",
            [("", String(snapshot.dataInBytes))]
        )
        metric(
            "http_requests_by_status_total", "Total requests by HTTP status code class.", "counter",
            [
        ...
        metric(
            "http_requests_by_method_total", "Total requests by HTTP method.", "counter",
            snapshot.methodCounts
                .sorted { $0.key < $1.key }
                .map { ("{method=\"\(Self.escapeLabel($0.key))\"}", String($0.value)) }
        )
        metric(
            "http_route_requests_total", "Total requests per route.", "counter",
            snapshot.routes.map { ("{route=\"\(Self.escapeLabel($0.path))\"}", String($0.requests)) }
        )
        metric(
            "http_route_errors_total", "Total errored requests per route.", "counter",
            snapshot.routes.map { ("{route=\"\(Self.escapeLabel($0.path))\"}", String($0.errors)) }
        )
        metric(
            "http_route_duration_seconds_avg", "Average request duration per route.", "gauge",
            snapshot.routes.map {
                ("{route=\"\(Self.escapeLabel($0.path))\"}", String(format: "%.6f", $0.averageDuration))
            }
        )
        metric(
            "process_uptime_seconds", "Time since the server started.", "gauge",
